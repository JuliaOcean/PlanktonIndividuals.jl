...
xg = ncread(fieldroot*"UVEL_big.nc","xG"); # Cell corner point long..
yg = ncread(fieldroot*"VVEL_big.nc","yG"); # Cell corner point lati..
zgrid = zf[1:end-1] .- zf[2:end];
xgrid = xg[2:end] .- xg[1:end-1];
ygrid = yg[2:end] .- yg[1:end-1];
# SETUP
#bdry = [[1 400]; [1 300]; [1 30]]; # boundaries of double grids
bdry = [[1 201]; [1 151]; [1 30]]; # boundaries of big grids
