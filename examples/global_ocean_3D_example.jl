...
!!! note
    This simulation may take several hours to complete a one-year run. Here we only run a few time steps.
...
begin
	...
	pth=OceanStateEstimation.ScratchSpaces.OCCA
end
...
# ╔═╡ e3a11fe6-da49-499e-afe2-91cde6866681
begin
	uvel = []; vvel = []; wvel = []; temp = [];
	for i in 1:12
		U = zeros(360, 160, 50)
		V = zeros(360, 161, 50)
		W = zeros(360, 160, 51)
		U[:,:,:] .= rd(joinpath(pth,"DDuvel.0406clim.nc"),"u",i)
		V[:,1:160,:] .= rd(joinpath(pth,"DDvvel.0406clim.nc"),"v",i)
		W[:,:,2:51] .= rd(joinpath(pth,"DDwvel.0406clim.nc"),"w",i)
		T=rd(joinpath(pth,"DDtheta.0406clim.nc"),"theta",i)
		
		push!(uvel, U); push!(vvel, V); push!(wvel, W), push!(temp, T);
	end
	uvels = cat(uvel..., dims=4)
...
begin
	✔2
	sim = PlanktonSimulation(model, ΔT = 3600.0,
									iterations = 5,#24*30*12,
									vels=(u=uvels, v=vvels, w=wvels),
									temp = temps,
									ΔT_vel=3600.0*24*30,
									ΔT_temp=3600.0*24*30)
end
...
manifest_format = "2.0"
project_hash = "7dc4395bb494ab207d449cdff2b4b8720b119619"
...
deps = ["Dataverse", "Distributed", "Glob", "JLD2", "MeshArrays", "Pkg", "Printf", "Scratch", "SharedArrays", "Statistics", "TOML"]
git-tree-sha1 = "d05605beeee42a9c3bef785829211e454a4ff213"
...
deps = ["Adapt", "CUDA", "JLD2", "KernelAbstractions", "LinearAlgebra", "Pkg", "Printf", "Random", "Serialization", "Statistics", "StructArrays"]
git-tree-sha1 = "85980492e9b173b05152a97041d6bc698edfd27f"
