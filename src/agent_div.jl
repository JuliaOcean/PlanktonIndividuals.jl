...
function agent_advection(phyts_a,vels,g,ΔT::Int64,grid_type::String)
    for i in 1:size(phyts_a,1)
        ...
        dx = uvel/g.dxF[xi,1]*ΔT # unit: grid/h
        dy = vvel/g.dyF[1,yi]*ΔT # unit: grid/h
        dz = wvel/g.dzF[zi]*ΔT # vertical movement, plus sinking, unit: grid/h
        phyt.x = phyt.x + dx
        phyt.y = phyt.y + dy
        phyt.z = max(1.1,min(g.Nz-0.1,phyt.z + dz))
        # periodic domain
...
function agent_advectionRK4(phyts_a, vel_field, g, ΔT::Int64, grid_type::String)
    for i in 1:size(phyts_a,1)
        ...
        gx1 = periodic_domain(g.Nx, phyt.x + u1/g.dxF[xi1,1]*0.5*ΔT)
        gy1 = periodic_domain(g.Ny, phyt.y + v1/g.dyF[1,yi1]*0.5*ΔT)
        ...
        gx2 = periodic_domain(g.Nx, phyt.x + u2/g.dxF[xi2,1]*0.5*ΔT)
        gy2 = periodic_domain(g.Ny, phyt.y + v2/g.dyF[1,yi2]*0.5*ΔT)
        ...
        gx3 = periodic_domain(g.Nx, phyt.x + u3/g.dxF[xi3,1]*0.5*ΔT)
        gy3 = periodic_domain(g.Ny, phyt.y + v3/g.dyF[1,yi3]*0.5*ΔT)
        ...
        phyt.x = periodic_domain(g.Nx, phyt.x + dx)
        phyt.y = periodic_domain(g.Ny, phyt.y + dy)
        phyt.z = phyt.z + dz
...
function agent_diffusionH(phyts,g,κh)
    for i in 1:size(phyts,1)
        phyt = phyts[i,:]
        xi, yi = trunc(Int,phyt.x),trunc(Int,phyt.y)
        phyt.x += rand(Uniform(-1.0,1.0)) * κh / g.dxF[xi,1]
        phyt.y += rand(Uniform(-1.0,1.0)) * κh / g.dyF[1,yi]
        phyt.x = periodic_domain(g.Nx, phyt.x)
        phyt.y = periodic_domain(g.Ny, phyt.y)
    end
end
...
function agent_diffusionV(phyts,g,κv)
    for i in 1:size(phyts,1)
        phyt = phyts[i,:]
        zi= trunc(Int,phyt.z)
        phyt.z += rand(Uniform(-1.0,1.0)) * κv / g.dzF[zi]
        phyt.z = max(1.1, min(g.Nz-0.1, phyt.z))
    end
end
