function construct_plankton(arch::Architecture, sp::Int, params::Dict, maxN::Int, FT::DataType)
    rawdata = StructArray(x    = zeros(FT, maxN), y    = zeros(FT, maxN), z    = zeros(FT, maxN),
                          ...
                          PRO  = zeros(FT, maxN), PRO_P = zeros(FT, maxN), PRO_T = zeros(FT, maxN),
                          PRO_R = zeros(FT, maxN), PRO_M = zeros(FT, maxN),
                          qFe_P = zeros(FT, maxN),
                          ...
                          S_RNA= zeros(FT, maxN), exu  = zeros(FT, maxN), ptc  = zeros(FT, maxN),
                          Rptc = zeros(FT, maxN),
                          graz = zeros(FT, maxN), mort = zeros(FT, maxN), dvid = zeros(FT, maxN)
                          ) 
    data = replace_storage(array_type(arch), rawdata)

    param_names=(:Nsuper, :C_DNA, :var, :PRO_P2DNA, :PRO_T2DNA, :PRO_R2DNA, :PRO_M2DNA, :RNA2DNA, :CH2DNA, :Chl2DNA,
                 :α, :Φ, :Topt, :Tmax, :Ea,
                 :PCmax, :VDOCmax, :VNO3max, :VNH4max, :VPO4max, :KsatDOC, :KsatNH4, :KsatNO3,
                 :KsatPO4, :CHmax, :CH_min, :NSTmax, :PSTmax,
                 :Chl2N, :R_NC_PRO_P, :R_NC_PRO_T, :R_NC_PRO_R, :R_NC_PRO_M, :R_NC_DNA, :R_NC_RNA, :R_PC_DNA, :R_PC_RNA, :respir,
                 :k_pro, :k_sat_pro, :k_rna, :k_sat_rna, :k_dna, :k_sat_dna,
                 :qFemax, :KsatFe, :R_Fe_PRO_P,
...
function initialize_plankton!(plank, N::Int, g::AbstractGrid, arch::Architecture)
    ...
    PRO_T2DNA = plank.p.PRO_T2DNA
    PRO_R2DNA = plank.p.PRO_R2DNA
    PRO_M2DNA = plank.p.PRO_M2DNA
    ...
    rand!(rng_type(arch), plank.data.PRO_T)
    rand!(rng_type(arch), plank.data.PRO_R)
    rand!(rng_type(arch), plank.data.PRO_M)
    rand!(rng_type(arch), plank.data.qFe_P)
    ...
    plank.data.PRO_T .= plank.data.PRO_T .* var .* 2.0f0 .+ 1.0f0 .- var  # range: (1.0-var,1.0+var)
    plank.data.PRO_R .= plank.data.PRO_R .* var .* 2.0f0 .+ 1.0f0 .- var  # range: (1.0-var,1.0+var)
    plank.data.PRO_M .= plank.data.PRO_M .* var .* 2.0f0 .+ 1.0f0 .- var  # range: (1.0-var,1.0+var)
    plank.data.qFe_P .= plank.data.qFe_P .* var .* 2.0f0 .+ 1.0f0 .- var  # range: (1.0-var,1.0+var)
    ...
    plank.data.PRO_P .= plank.data.PRO_P .* C_DNA .* Nsuper .* plank.data.ac .* PRO_P2DNA            # PRO_P mmolC/individual
    plank.data.PRO_T .= plank.data.PRO_T .* C_DNA .* Nsuper .* plank.data.ac .* PRO_T2DNA            # PRO_T mmolC/individual
    plank.data.PRO_R .= plank.data.PRO_R .* C_DNA .* Nsuper .* plank.data.ac .* PRO_R2DNA            # PRO_R mmolC/individual
    plank.data.PRO_M .= plank.data.PRO_M .* C_DNA .* Nsuper .* plank.data.ac .* PRO_M2DNA            # PRO_M mmolC/individual
    plank.data.qFe_P .= plank.data.qFe_P .* plank.data.PRO_P .* p.qFemax                                  # qFe_P mmolFe/individual
    plank.data.PRO .= plank.data.PRO_P .+ plank.data.PRO_T .+ plank.data.PRO_R .+ plank.data.PRO_M
