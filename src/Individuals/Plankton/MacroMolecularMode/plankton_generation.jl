function construct_plankton(arch::Architecture, sp::Int, params::Dict, maxN::Int, FT::DataType)
    rawdata = StructArray(x    = zeros(FT, maxN), y    = zeros(FT, maxN), z    = zeros(FT, maxN),
                          ...
                          PRO_P= zeros(FT, maxN), PRO_B= zeros(FT, maxN), DNA  = zeros(FT, maxN), RNA  = zeros(FT, maxN), 
                          ...
                          resp = zeros(FT, maxN), S_PRO_P = zeros(FT, maxN), S_PRO_B = zeros(FT, maxN), S_DNA= zeros(FT, maxN),
    ...
    param_names=(:Nsuper, :C_DNA, :var, :PRO_P2DNA, :PRO_B2DNA, :RNA2DNA, :CH2DNA, :Chl2DNA,
                 :α, :Φ, :Topt, :Tmax, :Ea,
                 :PCmax, :VDOCmax, :VNO3max, :VNH4max, :VPO4max, :KsatDOC, :KsatNH4, :KsatNO3,
                 :KsatPO4, :CHmax, :NSTmax, :PSTmax,
                 :Chl2N, :R_NC_PRO_P, :R_NC_PRO_B, :R_NC_DNA, :R_NC_RNA, :R_PC_DNA, :R_PC_RNA, :respir,
                 :k_pro_P, :k_pro_B, :k_sat_pro, :k_rna, :k_sat_rna, :k_dna, :k_sat_dna, 
                 :CH_min,
...
function initialize_plankton!(plank, N::Int, g::AbstractGrid, arch::Architecture)
    ...
    rand!(rng_type(arch), plank.data.PRO_B)
    ...
    plank.data.PRO_P .= plank.data.PRO_P .* var .* 2.0f0 .+ 1.0f0 .- var
    plank.data.PRO_B .= plank.data.PRO_B .* var .* 2.0f0 .+ 1.0f0 .- var
    ...
    plank.data.PRO_B .= plank.data.PRO_B .* C_DNA .* Nsuper .* plank.data.ac .* PRO_B2DNA                    # PRO_B mmolC/individual
...
@inline function total_C_biomass(PRO_P, PRO_B, DNA, RNA, CH, Chl)
    C_tot = PRO_P + PRO_B + DNA + RNA + CH + Chl / 893.49f0 * 55.0f0 
    return C_tot
end
@inline function total_N_biomass(PRO_P, PRO_B, DNA, RNA, NST, Chl, p)
    N_tot = PRO_P * p.R_NC_PRO_P + PRO_B * p.R_NC_PRO_B + DNA * p.R_NC_DNA + RNA * p.R_NC_RNA + NST + Chl / 893.49f0 * 4.0f0
    return N_tot
end
