...
@kernel function calc_consume_kernel!(ctsdic, ctsnh4, ctsno3, ctspo4, ctsfet, 
                                        plank, ac, x, y, z, ΔT)
    i = @index(Global)
    ...
    @inbounds KernelAbstractions.@atomic ctsfet[x[i], y[i], z[i]] += -plank.VFe[i]  * ΔT * ac[i]
end
function calc_consume!(ctsdic, ctsnh4, ctsno3, ctspo4, ctsfet, plank, ac, x, y, z, ΔT, arch)
    kernel! = calc_consume_kernel!(device(arch), 256, (size(ac,1)))
    kernel!(ctsdic, ctsnh4, ctsno3, ctspo4, ctsfet, plank, ac, x, y, z, ΔT)
    return nothing 
end

##### deal with grazed or dead individuals
@kernel function calc_loss_kernel!(ctsnh4, ctsno3, ctsdoc, ctspoc, ctsdon, ctspon, 
                                    ctsdop, ctspop, ctsdofe, ctspofe,
                                    plank, ac, x, y, z, loss, 
    ...
    @inbounds KernelAbstractions.@atomic ctspofe[x[i], y[i], z[i]] += plank.qFe[i] * (1.0f0-lossFracFe) * ac[i] * loss[i]
end
function calc_loss!(ctsnh4, ctsno3, ctsdoc, ctspoc, ctsdon, ctspon, ctsdop, ctspop, ctsdofe, ctspofe, 
    ...
    kernel!(ctsnh4, ctsno3, ctsdoc, ctspoc, ctsdon, ctspon, ctsdop, ctspop, ctsdofe, ctspofe, 
