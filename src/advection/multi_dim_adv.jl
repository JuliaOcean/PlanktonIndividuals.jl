...
@kernel function multi_dim_x_kernel!(ctemp, Gc, c, u, g::Grids, ΔT)
    i, j, k = @index(Global, NTuple)
    ### offset index for halo points
    ii = i + g.Hx
    jj = j + g.Hy
...
function multi_dim_x!(nut_temp, Gcs, nut, u, g::Grids, ΔT, arch::Architecture)
...
@kernel function multi_dim_y_kernel!(ctemp, Gc, c, v, g::Grids, ΔT)
    i, j, k = @index(Global, NTuple)
    ### offset index for halo points
    ii = i + g.Hx
    jj = j + g.Hy
...
function multi_dim_y!(nut_temp, Gcs, nut, v, g::Grids, ΔT, arch::Architecture)
...
@kernel function multi_dim_z_kernel!(ctemp, Gc, c, w, g::Grids, ΔT)
    i, j, k = @index(Global, NTuple)
    ### offset index for halo points
    ii = i + g.Hx
    jj = j + g.Hy
...
function multi_dim_z!(nut_temp, Gcs, nut, w, g::Grids, ΔT, arch::Architecture)
