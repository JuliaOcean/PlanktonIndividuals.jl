...
function PlanktonSimulation(model::PlanktonModel; ΔT::Int64, nΔT::Int64,
    ...
    if vels ≠ (;)
        ...
        if size(vels.u)[4] < floor(Int,nΔT*ΔT/ΔT_vel)
            throw(ArgumentError("Velocities provided cannot cover the time period of (nΔT+1)*ΔT ($(nΔT*ΔT) seconds) with ΔT_vel = $(ΔT_vel)."))
        end
    end
    ...
    input = PlanktonInput(temp, PARF, vels, ΔT_vel)

    validate_bcs(model.nutrients, model.grid, nΔT)

    sim = PlanktonSimulation(model, input, diags, ΔT, nΔT, res_dir, save_individuals)
