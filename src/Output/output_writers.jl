...
- `abiotic_particle_prefix`: Descriptive filename prefixed to abiotic_particle output files.
- `colony_prefix`: Descriptive filename prefixed to colony output files.
...
- `save_colony`: write colony to disk, default: `false`.
- `colony_include`: list of colony properties to save, default: `(:x, :y, :z, :Sz)`.
- `colony_iteration_interval`: The time interval that colony are saved, 1 timestep by default.
...
function PlanktonOutputWriter(;dir = "./results",
    ...
    if save_colony
        colony_file = joinpath(dir, colony_prefix*".jld2")
    end

    return PlanktonOutputWriter(dir, write_log, save_diags, save_phytoplankton,
                                save_abiotic_particle, save_colony,
                                ...
                                colony_file, colony_include, colony_iteration_interval,
                                max_filesize, 1, 1, 1, 1)
end
...
function save_colony_string(writer::PlanktonOutputWriter)
    if writer.save_colony
        return string("colony is saved as $(writer.acolony_file)\n",
                      "│   ├── every $(writer.acolony_iteration_interval) seconds\n",
                      "│   └── including: $(writer.acolony_include)")
    else
        return "colony are not saved"
    end
end
