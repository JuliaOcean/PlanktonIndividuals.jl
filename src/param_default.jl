...
param_default=Dict(
    ...
    "Chl2Cint" => [0.16,0.16],        # Initial Chla:C ratio in phytoplankton (gChl/molC)
    "R_NC"     => 16/106,             # N:C ratio in cell biomass
    "R_PC"     => 1/106,              # N:C ratio in cell biomass
    "α"        => [2.0e-2,2.0e-2],    # Irradiance absorption coeff (m^2/mgChl)
    ...
    "a_β"      => [3.1,3.1],          # scale parameter for metabolic partitioning of biosynthesis
    "b_β"      => [-3.8,-3.8],        # shape parameter for metabolic pratitioning of biosynthesis
