...
function phyt_params_default(N::Int64, mode::ProteinMode)
    params=Dict(
        ...
        "var"        => [0.3],     # Variance of the normal distribution of initial phyto individuals
        "RNA2DNA"    => [1.73],    # Initial RNA:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "PRO_RB2DNA" => [0],       # Initial ribosomal protein:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "PRO_MC2DNA" => [1.0],     # Initial metabolic protein:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "PRO_MN2DNA" => [1.0],     # Initial nitrogen metabolism protein:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "PRO_TN2DNA" => [1.0],     # Initial nitrate transporter:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "PRO_TP2DNA" => [1.0],     # Initial phosphate transporter:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "PRO_TFe2DNA"=> [1.0],     # Initial iron transporter:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "PRO_RS2DNA" => [1.0],     # Initial carbon metabolism protein:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "PRO_PS2DNA" => [1.0],     # Initial photosynthesis protein:DNA ratio in phytoplankton (mmol C/mmolC)
        "CH2DNA"     => [23.3],    # Initial carbohydrate+lipid:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "Chl2DNA"    => [3.45],    # Initial Chla:DNA ratio in phytoplankton (mmolC/mmolC) from Micromonas sp.
        "α"          => [4.5e-2],  # Irradiance absorption coeff (mmolC m² second/mgChl /μmol photon)
        "Topt"       => [27.0],    # Optimal temperature for growth (C)
        ...
        "PCmax"      => [7.8e-8],  # Maximum light harvesting rate (mmolATP/mmolC/second)
        "VDOCmax"    => [0.0],     # Maximum DOC uptake rate (mmol C/mmol C/second)
        "KcatCF"     => [1.0],     # Maximum carbon fixation rate per carbon fixation protein (per second)
        "KcatNF"     => [1.0],     # Maximum N fixation rate per N fixation protein (mmolN/mmolC/second)
        "KcatNR"     => [1.0],     # Maximum nitrate reduction rate per nitrate reduction protein (mmolN/mmolC/second)
        "KcatTNH4"   => [1.0],     # Maximum ammonium uptake rate per NH4 transporter protein (mmolN/mmolC/second)
        "KcatTNO3"   => [1.0],     # Maximum nitrate uptake rate per NO3 transporter protein (mmolN/mmolC/second)
        "KcatTPO4"   => [1.0],     # Maximum phosphate uptake rate per PO4 transporter protein (mmolP/mmolC/second)
        "KcatTFe"    => [1.0],     # Maximum iron uptake rate per Fe transporter protein (mmolFe/mmolC/second)
        "KcatRS"     => [1.0],     # Maximum respiration rate per respiration protein (per second)
        "r_max"      => [1.0],     # Maximum Maximal elongation rate (aa/per second/per molecule)
        "n_MC"       => [1.0],     # Length of C metabolic enzyme complex (aa/ per molecule)
        "n_MN"       => [1.0],     # Length of N metabolic enzyme complex (aa/ per molecule)
        "n_RB"       => [1.0],     # Length of ribosome (aa/ per molecule)
        "n_TN"       => [1.0],     # Length of ammonium transporter (aa/ per molecule)
        "n_TPO4"     => [1.0],     # Length of phosphate transporter (aa/ per molecule)
        "n_TFe"      => [1.0],     # Length of iron transporter
        "n_RS"       => [1.0],     # Length of respiration enzyme complex (aa/ per molecule)
        "n_PS"       => [1.0],     # Length of photosynthesis enzyme complex (aa/ per molecule)
        "KsatNH4"    => [0.005],   # Half-saturation coeff (mmol N/m³)
        "KsatNO3"    => [0.010],   # Half-saturation coeff (mmol N/m³)
        "KsatPO4"    => [0.003],   # Half-saturation coeff (mmol P/m³)
        "KsatDOC"    => [0.0],     # Half-saturation coeff (mmol C/m³)
        ...
        "TN_max"     => [1.0],     # Maximum nitrate concentration for protein synthesis in cell (mmol N/m3)
        "TP_max"     => [1.0],     # Maximum phosphate concentration for protein synthesis in cell (mmol P/m3)
        "TFe_max"    => [1.0e-3],  # Maximum iron concentration for protein synthesis in cell (mmol Fe/m3)
        ...
        "PRO_RBmin"  => [0.0],     # Minimum ribosome protein quota (mmol C/individual)
        "PRO_PSmin"  => [0.0],     # Minimum photosynthesis protein quota (mmol C/individual)
        "PRO_MCmin"  => [0.0],     # Minimum carbon fixation protein quota (mmol C/individual)
        "Chlmin"     => [0.0],     # Minimum Chl quota (mg Chl/individual)
        "e_TNO3"     => [1.0],     # Energy consumption rate of nitrate transporter (mmolATP/mmolN)
        "e_TPO4"     => [1.0],     # Energy consumption rate of phosphate transporter (mmolATP/mmolP)
        "e_TFe"      => [1.0],     # Energy consumption rate of iron transporter (mmolATP/mmolFe)
        "e_CF"       => [9.0],     # Energy consumption rate of carbon fixation (mmolATP/mmolC)
        "e_RS"       => [5.0],     # Energy production rate of respiration (mmolATP/mmolC)
        "e_NF"       => [8.0],     # Energy consumption rate of N fixation (mmolATP/mmolN)
        "e_NR"       => [10.0],    # ??Energy consumption rate of NO3 reduction (mmolATP/mmolN)
        "e_SP"       => [1.0],     # Energy consumption rate of protein synthesis (mmolATP/mmol C)
        "e_DNA"      => [1.0],     # Energy consumption rate of DNA synthesis (mmolATP/mmol C)
        "e_RNA"      => [1.0],     # Energy consumption rate of RNA synthesis (mmolATP/mmol C)
        "e_min"      => [0.0],     # Minimum maintenance energy requirement (mmolATP/individual/second)
        ...
        "R_NC_RNA"   => [1/2.8],   # N:C ratio in RNA (from Inomura et al 2020.)
        "R_PC_RNA"   => [1/10.7],  # P:C ratio in RNA
        "R_C_RNAPRB" => [0],       # C:C ratio in RNA/ ribosome protein
        "dvid_P"     => [1.0e-5],  # Division probability per second
        "grz_P"      => [0.0],     # Grazing probability per second
        "mort_P"     => [5e-5],    # Probability of cell natural death per second
        "mort_reg"   => [0.5],     # Regulation of cell natural death
