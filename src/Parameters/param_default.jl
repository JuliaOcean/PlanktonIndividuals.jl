...
function phyt_params_default(N::Int64, mode::IronEnergyMode)
    params=Dict(
        ...
        "e_cf"      => [9.0e-5],  # Energy consumption rate of carbon fixation (mmolATP/mmolC)
        "e_rs"      => [5.0e-5],  # Energy production rate of respiration (mmolATP/mmolC)
        "e_nf"      => [8.0e-5],  # Energy consumption rate of N fixation (mmolATP/mmolN)
        "e_nr"      => [10.0e-5], # ??Energy consumption rate of NO3 reduction (mmolATP/mmolN)
