...
function phyt_params_default(N::Int64, mode::ProteinMode)
    params=Dict(
        ...
        "PRO_RS2DNA" => [0.0],   # Initial carbon metabolism protein:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "PRO_P2DNA" => [0.0],    # Initial photosynthesis protein:DNA ratio in phytoplankton (mmol C/mmolC)
        "CH2DNA"   => [23.3],    # Initial carbohydrate+lipid:DNA ratio in phytoplankton (mmol C/mmolC) from Micromonas sp.
        "Chl2DNA"  => [3.45],    # Initial Chla:DNA ratio in phytoplankton (mmolC/mmolC) from Micromonas sp.
        "α"        => [4.5e-2],  # Irradiance absorption coeff (mmolC m² second/mgChl /μmol photon)
        "Topt"     => [27.0],    # Optimal temperature for growth (C)
        ...
        "PCmax"    => [7.8e-8],  # Maximum light harvesting rate (mmolATP/mmolC/second)
        "VDOCmax"  => [0.0],     # Maximum DOC uptake rate (mmol C/mmol C/second)
        "KcatCF"   => [0.0],     # Maximum carbon fixation rate per carbon fixation protein (per second)
        "KcatNF"   => [0.0],     # Maximum N fixation rate per N fixation protein (mmolN/mmolC/second)
        "KcatNR"   => [0.0],     # Maximum nitrate reduction rate per nitrate reduction protein (mmolN/mmolC/second)
        "KcatTNH4" => [0.0],     # Maximum ammonium uptake rate per NH4 transporter protein (mmolN/mmolC/second)
        "KcatTNO3" => [0.0],     # Maximum nitrate uptake rate per NO3 transporter protein (mmolN/mmolC/second)
        "KcatTPO4" => [0.0],     # Maximum phosphate uptake rate per PO4 transporter protein (mmolP/mmolC/second)
        "KcatTFe"  => [0.0],     # Maximum iron uptake rate per Fe transporter protein (mmolFe/mmolC/second)
        "KcatRS"   => [0.0],     # Maximum respiration rate per respiration protein (per second)
        ...
        "n_rs"     => [0.0],     # Length of respiration enzyme complex (aa/ per molecule)
        "n_p"      => [0.0],     # Length of photosynthesis enzyme complex (aa/ per molecule)
        "KsatNH4"  => [0.005],   # Half-saturation coeff (mmol N/m³)
        "KsatNO3"  => [0.010],   # Half-saturation coeff (mmol N/m³)
        "KsatPO4"  => [0.003],   # Half-saturation coeff (mmol P/m³)
        "KsatDOC"  => [0.0],     # Half-saturation coeff (mmol C/m³)
        "KsatNR"   => [0.0],     # Half-saturation coeff (mmol N / mmolC)
        "KsatFe"   => [0.0],     # Half-saturation coeff of iron quota for iron transport (mmolFe/m³)
        "KFe_em"   => [0.0],     # Half-saturation coeff of iron quota for photosynthesis, nitrogen fixation, nitrate reduction and respiration (mmolFe/mmolC)
        "TN_max"   => [0.0],     # Maximum nitrate concentration for protein synthesis in cell (mmol N/m3)
        "TP_max"   => [0.0],     # Maximum phosphate concentration for protein synthesis in cell (mmol P/m3)
        "TFe_max"  => [0.0],     # Maximum iron concentration for protein synthesis in cell (mmol Fe/m3)
        "NSTmax"   => [0.12],    # Maximum N reserve in total N (mmol N/mmol N)
        "PSTmax"   => [0.70],    # Maximum P reserve in total P (mmol P/mmol P)
        "CHmax"    => [0.4],     # Maximum Carbohydrate in cell (mmol C/mmol C)
        "qNH4max"  => [0.25],    # Maximum NH4 quota in cell (mmolN/mmolC)
        "qNO3max"  => [0.25],    # Maximum NO3 quota in cell (mmolN/mmolC)
        "qFemax"   => [2.0e-5],  # Maximum Fe quota in cell (mmolFe/mmolC)
        "k_degr"   => [2.8e-5],  # Ribosome protein degradation rate (per second)
        "k_degp"   => [1.9e-5],  # photosynthesis protein degradation rate (per second)
        ...
        "PRO_Rmin" => [0.0],     # Minimum ribosome protein quota (mmol C/individual)
        "PRO_Pmin" => [0.0],     # Minimum photosynthesis protein quota (mmol C/individual)
        "PRO_Mcmin"=> [0.0],     # Minimum carbon fixation protein quota (mmol C/individual)
        "Chlmin"   => [0.0],     # Minimum Chl quota (mg Chl/individual)
        "e_tno3"   => [0.0],     # Energy consumption rate of nitrate transporter (mmolATP/mmolN)
        "e_tpo4"   => [0.0],     # Energy consumption rate of phosphate transporter (mmolATP/mmolP)
        "e_tfe"    => [0.0],     # Energy consumption rate of iron transporter (mmolATP/mmolFe)
        "e_cf"     => [9.0],     # Energy consumption rate of carbon fixation (mmolATP/mmolC)
        "e_rs"     => [5.0],     # Energy production rate of respiration (mmolATP/mmolC)
        "e_nf"     => [8.0],     # Energy consumption rate of N fixation (mmolATP/mmolN)
        "e_nr"     => [10.0],    # ??Energy consumption rate of NO3 reduction (mmolATP/mmolN)
        "e_sp"     => [0.0],     # Energy consumption rate of protein synthesis (mmolATP/mmol C)
        "e_dna"    => [0.0],     # Energy consumption rate of DNA synthesis (mmolATP/mmol C)
        "e_rna"    => [0.0],     # Energy consumption rate of RNA synthesis (mmolATP/mmol C)
