...
function phyt_params_default(N::Int64, mode::IronEnergyMode)
    params=Dict(
        ...
        "k_Fe_ST2PS"=> [4.6e-6],  # Allocation rate of Fe from storage to PS (per second)
        "k_Fe_PS2ST"=> [4.6e-6],  # Allocation rate of Fe from PS to storage (per second)
        "k_Fe_ST2NR"=> [4.6e-6],  # Allocation rate of Fe from storage to NR (per second)
        "k_Fe_NR2ST"=> [4.6e-6],  # Allocation rate of Fe from NR to storage (per second)
