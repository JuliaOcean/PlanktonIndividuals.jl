...
function phyt_update(t::Int64, ΔT::Int64, phyts_a, model)
    ...
    for i in 1:size(phyts_a,1)
        ...
        reg_size = max(0.0, phyt.size - model.params["dvid_size"])
        shape_factor_divide = (model.params["a_dvi"][sp]*reg_size)^model.params["b_dvi"][sp]
        ...
        reg_age = max(0.0, phyt.age - model.params["death_age"])
        shape_factor_death = model.params["a_death"]*reg_age^model.params["b_death"]
        P_death = rand(Bernoulli(shape_factor_death/(1+shape_factor_death)))

        # Compute extra cost for biosynthesis, return a rate (per hour)
        respir_extra = model.params["respir_ex"]*phyt.size^model.params["respir_b"] 

        # Compute light attenuation
        atten = (model.params["katten_w"] + model.params["katten_c"] * cumsum_chl[x, y, z])*(-g.zF[z])
        if atten == 0.0
            α_I = model.params["α"]*IR_t
        else
            α_I = model.params["α"]*IR_t*(1.0 - exp(-atten))/atten
        end

        # Compute photosynthesis rate
        Tempstd = exp(model.params["TempAe"]*(1.0/(temp_t+273.15)-1.0/model.params["Tempref"]))
        photoTempFunc = model.params["TempCoeff"]*max(1.0e-10,Tempstd)
        PCmax_sp = model.params["PCmax"][phyt.sp]/86400
        PCm = PCmax_sp*photoTempFunc*phyt.size^model.params["PC_b"][phyt.sp]
        PC = PCm*(1-exp(-α_I*phyt.chl/(phyt.Cq2*PCm)))
        PS = PC*phyt.Cq2 # unit: mmol C/second/individual
        Eₖ = PCm/(phyt.chl/phyt.Cq2*model.params["α"])
        tmp = α_I/model.params["α"]
        if (tmp > Eₖ) & (model.params["inhibcoef"][phyt.sp] == 1.0)
            PS = PS*Eₖ/tmp*model.params["inhibcoef"][phyt.sp]
        end
        ...
        regQ = max(0.0,min(1.0,(model.params["Nqmax"]-Qn)/(model.params["Nqmax"]-model.params["Nqmin"])))
        VNmax_sp = model.params["VNmax"][phyt.sp]/86400
        VNm = VNmax_sp*phyt.size^model.params["VN_b"][phyt.sp]
        Nuptake = VNm*DIN/(DIN+model.params["KsatN"][phyt.sp])*regQ
        ...
        if IR_t > 0
            ρ_chl = PS/(model.params["α"]*IR_t*phyt.chl/phyt.Cq2)
        else
            ρ_chl = 0.0
        end

        # Metabolic partitioning for biosynthesis, decrease with size
        shape_factor_β = model.params["a_β"]*phyt.size^model.params["b_β"]
        β = shape_factor_β/(1+shape_factor_β) 

        if P_graz == false #not grazed
            SynC = min(VN/model.params["R_NC"],β*model.params["k_mtb"]*phyt.Cq1/(1+respir_extra))
            CostC = SynC*(1+respir_extra)
            MaintenC = (1-β)*model.params["k_mtb"]*phyt.Cq1
            ...
            phyt.chl = phyt.chl + ρ_chl*VN*model.params["Chl2N"]
            ...
            if P_death == false # not natural death
                ...
                consume.DOC[x, y, z] = consume.DOC[x, y, z] + (phyt.Cq1+phyt.Cq2)*model.params["mortFracC"]
                consume.DON[x, y, z] = consume.DON[x, y, z] + phyt.Nq*model.params["mortFracN"]
                consume.POC[x, y, z] = consume.POC[x, y, z] + (phyt.Cq1+phyt.Cq2)*(1.0 - model.params["mortFracC"])
                consume.PON[x, y, z] = consume.PON[x, y, z] + phyt.Nq*(1.0 - model.params["mortFracN"])
                death_ct += 1
            end # naturan death
            ...
            graz_ct += 1
            consume.DOC[x, y, z] = consume.DOC[x, y, z] + (phyt.Cq1+phyt.Cq2)*model.params["grazFracC"]*0.5
            consume.DON[x, y, z] = consume.DON[x, y, z] + phyt.Nq*model.params["grazFracN"]*0.5
            consume.POC[x, y, z] = consume.POC[x, y, z] + (phyt.Cq1+phyt.Cq2)*(1.0 - model.params["grazFracC"])*0.5
            consume.PON[x, y, z] = consume.PON[x, y, z] + phyt.Nq*(1.0 - model.params["grazFracN"])*0.5
        end # graze
