...
function gen_agents(::CPUs,RunParam::RunParams,grid)
    ...
    for i in 1:Nsp
        lower = Int(1+(i-1)*N)
        upper = Int(N+(i-1)*N)
        phyts0[lower:upper,6]  .= Cquota[i]*Nsuper                             # Bm
        phyts0[lower:upper,11] .= i                                            # species
    end
    phyts0[:,6] .= phyts0[:,6] .* phyts0[:,5]                                  # Bm
    for i in 1:Nsp
        lower = Int(1+(i-1)*N)
        upper = Int(N+(i-1)*N)
        phyts0[lower:upper, 7] .= copy(phyts0[lower:upper,6]) .* rand(Uniform(cqmin[i],cqmax[i]),N) # Cq
        phyts0[lower:upper, 8] .= copy(phyts0[lower:upper,6]) .* rand(Uniform(nqmin[i],nqmax[i]),N) # Nq
        phyts0[lower:upper, 9] .= copy(phyts0[lower:upper,6]) .* rand(Uniform(pqmin[i],pqmax[i]),N) # Pq
        phyts0[lower:upper,10] .= copy(phyts0[lower:upper,6]) .* params["Chl2Cint"][i]              # Chl
    end
...
function gen_agents(::GPUs,RunParam::RunParams,grid)
    ...
    for i in 1:Nsp
        lower = Int(1+(i-1)*N)
        upper = Int(N+(i-1)*N)
        phyts0[lower:upper, 7] .= copy(phyts0[lower:upper,6]) .* CuArray(rand(Uniform(cqmin[i],cqmax[i]),N)) # Cq
        phyts0[lower:upper, 8] .= copy(phyts0[lower:upper,6]) .* CuArray(rand(Uniform(nqmin[i],nqmax[i]),N)) # Nq
        phyts0[lower:upper, 9] .= copy(phyts0[lower:upper,6]) .* CuArray(rand(Uniform(pqmin[i],pqmax[i]),N)) # Pq
        phyts0[lower:upper,10] .= copy(phyts0[lower:upper,6]) .* params["Chl2Cint"][i]                       # Chl
    end
    phyts0[:,12] .= 1.0                                                        # generation
    phyts0[:,13] .= 1.0                                                        # age
