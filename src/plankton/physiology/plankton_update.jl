function grazing!(op_array, consume, arch::Architecture, g::Grids, p)
    grz_array = op_array[findall(x -> x == 1.0, op_array[:,29]), :]
    op_array  = op_array[findall(x -> x == 0.0, op_array[:,29]), :]
...
function mortality!(op_array, consume, arch::Architecture, g::Grids, p)
    mo_array = op_array[findall(x -> x == 1.0, op_array[:,30]), :]
    op_array = op_array[findall(x -> x == 0.0, op_array[:,30]), :]
...
function plankton_update!(phytos, consume, diags, arch::Architecture,
    ...
    find_NPT!(op, arch, NH4, NO3, PO4, DOC, par, temp, g,
              p["α"], p["Φ"], p["TempAe"], p["Tempref"], p["TempCoeff"])

    ##### Carbon uptake
    calc_PS!(op, arch, p["PCmax"], p["PC_b"])
    calc_VDOC!(op, arch, g, ΔT, p["Cqmax"], p["Cqmin"], p["VDOCmax"], p["VDOC_b"], p["KsatDOC"])

    ##### Nitrogen uptake
    calc_VN!(op, arch, g, ΔT,
             p["Nqmax"], p["Nqmin"], p["VNH4max"], p["VNO3max"], p["VN_b"], p["KsatNH4"], p["KsatNO3"], p["R_NC"])

    ##### Phosphorus uptake
    calc_VP!(op, arch, g, ΔT, p["Pqmax"], p["Pqmin"], p["VPO4max"], p["VP_b"], p["KsatPO4"], p["R_PC"])
