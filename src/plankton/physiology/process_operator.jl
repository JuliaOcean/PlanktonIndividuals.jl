...
@kernel function find_NPT_kernel!(plank, inds::AbstractArray{Int64,2},
    ...
    @inbounds plank[i,21] = max(1.0e-10, exp(TempAe * (1.0 / (temp[xi, yi, zi] + 273.15) - 1.0 / Tempref))) * TempCoeff
end
...
@kernel function calc_VP_kernel!(plank, g::Grids,
...
@kernel function get_rands_kernel!(plank)
    i = @index(Global, Linear)
    plank[i,31] = plank[i,58] ≥ plank[i,31] ? 0.0 : 1.0
    plank[i,32] = plank[i,59] ≥ plank[i,32] ? 0.0 : 1.0
    plank[i,33] = plank[i,60] ≥ plank[i,33] ? 0.0 : 1.0
end
