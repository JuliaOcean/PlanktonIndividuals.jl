...
#     kernel! = acc_counts_kernel!(device(arch), 256, (1,))
#     event = kernel!(ctschl, ctspop, chl, ac, x, y, z)
#     wait(device(arch), event)
#     return nothing
# end
# @kernel function acc_counts_kernel!(ctstmp1, ctstmp2, chl, ac, x, y, z)
#     i = @index(Global)
#     @inbounds ctstmp1[x[i], y[i], z[i], i] += chl[i] * ac[i]
#     @inbounds ctstmp2[x[i], y[i], z[i], i] += 1.0 * ac[i]
# end
# function acc_counts!(ctschl, ctspop, ctstmp1, ctstmp2, chl, ac, x, y, z, arch::Architecture)
#     kernel! = acc_counts_kernel!(device(arch), 256, (size(ac,1),))
#     ctstmp1 .= 0.0
#     ctstmp2 .= 0.0
#     event = kernel!(ctstmp1, ctstmp2, chl, ac, x, y, z)
#     wait(device(arch), event)
#     # sum!(ctschl,ctstmp1)
#     # sum!(ctspop,ctstmp2)
#     return nothing
# end
# @inline function gpu_acc_counts_kernel!(ctstmp1, ctstmp2, chl, ac, x, y, z)
#     index = (blockIdx().x - 1) * blockDim().x + threadIdx().x
#     stride = blockDim().x * gridDim().x
#     for i = index:stride:size(ac,1)
#         @inbounds ctstmp1[x[i], y[i], z[i], i] += chl[i] * ac[i]
#         @inbounds ctstmp2[x[i], y[i], z[i], i] += ac[i]
#     end
#     return nothing
# end
# @inline function cpu_acc_counts_kernel!(ctstmp1, ctstmp2, chl, ac, x, y, z)
#     for i in 1:size(ac,1)
#         @inbounds ctstmp1[x[i], y[i], z[i], i] += chl[i] * ac[i]
#         @inbounds ctstmp2[x[i], y[i], z[i], i] += ac[i]
#     end
...
# function acc_counts!(ctschl, ctspop, ctstmp1, ctstmp2, chl, ac, x, y, z, ::GPUs)
#     ctstmp1 .= 0.0
#     ctstmp2 .= 0.0
#     @cuda threads=256 blocks=ceil(Int, size(ac,1)/256) gpu_acc_counts_kernel!(ctstmp1, ctstmp2, chl, ac, x, y, z)
#     sum!(ctschl,ctstmp1)
#     sum!(ctspop,ctstmp2)
#     return nothing 
# end

# function acc_counts!(ctschl, ctspop, ctstmp1, ctstmp2, chl, ac, x, y, z, ::CPUs)
#     ctstmp1 .= 0.0
#     ctstmp2 .= 0.0
#     cpu_acc_counts_kernel!(ctstmp1, ctstmp2, chl, ac, x, y, z)
#     sum!(ctschl,ctstmp1)
#     sum!(ctspop,ctstmp2)
#     return nothing 
# end
