...
@inline δx_c2f(g::grid, f, i, j, k) = @inbounds f[i, j, k] - f[decmod1(i, g.Nx), j, k]
@inline δy_c2f(g::grid, f, i, j, k) = @inbounds f[i, j, k] - f[i, decmod1(j, g.Ny), k]
@inline function δz_c2f(g::grid, f, i, j, k)
    if k == 1
        return 0
    else
...
@inline function δx²_c2f2c(g::grid, f, i, j, k)
...
@inline function δy²_c2f2c(g::grid, f, i, j, k)
...
@inline function δz²_c2f2c(g::grid, f, i, j, k)
    if k == g.Nz
...
@inline function κ∇²(g::grid, Q, κh, κv, i, j, k)
    @inbounds ΔV = g.Δx[i, j, k] * g.Δy[i, j, k] * g.Δz[i, j, k]
